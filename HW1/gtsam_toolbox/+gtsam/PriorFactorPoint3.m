...
classdef PriorFactorPoint3 < gtsam.NoiseModelFactor
  ...
  methods
    function obj = PriorFactorPoint3(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(1425, varargin{2});
        end
        base_ptr = gtsam_wrapper(1424, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.Point3') && isa(varargin{3},'gtsam.noiseModel.Base')
        [ my_ptr, base_ptr ] = gtsam_wrapper(1426, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(1427, obj.ptr_gtsamPriorFactorPoint3);
    end
